...
class top(gr.top_block):
   ...
   def __init__(self, consume_amount):
      ...
      self.interp_fir_filter_xxx_0 = filter.interp_fir_filter_ccc(8, taps)
...
def time_it(tb, np_times, np_items, r):
   ...
   tb.run()
   tb.wait()
   np_times[r] = tb.interp_fir_filter_xxx_0.pc_work_time_avg()
   np_items[r] = tb.interp_fir_filter_xxx_0.pc_nproduced_avg()
...
if __name__ == "__main__":
   ...
   try:
      ...
      #sample_values = [2**i for i in range(1, 9)]
      sample_values = [i*64 for i in range(1, 8)]
      data = {
         "Samples Consumed": [],
         "Clock Cycles": []
      }
      ...
      print(df)
      df.to_csv("interpolating_filter.csv", index=False)
